...
class Wifi:
  def __init__(self):
    ...
    config.add('wifi', 'mode', selection, 'Access point only', 'ID for access point/router (0 - 32 characters letters and numbers)')
    config.add('wifi', 'ip', 'text', '', 'Static IP address (Ignoring DHCP from router)', True)
  ...
  async def start(self):
    while True:
      if platform == PC:
        ...
        # self.ap_if.ifcfg(('10.0.0.1', '255.255.255.0', '10.0.0.1', '0.0.0.0'))
      ...
      if platform != PC and not self.link_up and self.mode != self.MODE_AP and config['wifi']['ssid'].value:
        ...
        if status == network.STAT_GOT_IP:
          ...
          debug(f"Connection failed. Wrong password for {cfg['wifi']['ssid'].value}")
