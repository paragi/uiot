...
class Request:
  ...
  def body(self):
    if not len(self._body) and len(self._body_raw):
      encoding = None
      if 'Content-Type' in self.headers().keys():
        encoding = self.headers()['Content-Type']
      if encoding and encoding.startswith('application/x-www-form-urlencoded'):
        for line in self._body_raw.split('\r\n'):
          (key, value) = line.split('=', 1)
          self._body[key] = value
      elif encoding and encoding.startswith('application/json'):
        import json
...
class Webserver():
  ...
  async def handle_client(self, reader, writer):
    try:
      while True:  # Run once
        ...
        while True:
          try:
            ...
            print("Read timeout while reading headers from client request")
            break
          if line == b'\r\n': break
        ...
        # Serve dynamic content
          ...
          if body_length > 0:
            try:
              ...
              print("Read timeout while reading body")
              break
      ...
      print(f"uWebserver Error: {type(e).__name__} at line {e.__traceback__.tb_lineno}: {e}")
